from tests.MOF_builder.functions.frag_recognizer import create_lG,plot2dedge
...
def get_mapping_between_nometal_linker_xyz(linker_topic,center_frag_nodes_num,center_Xs,single_frag_nodes_num,frag_Xs,template_xyz, new_xyz='Residues/EDGE.xyz'):
    ...
    if linker_topic == 2:
        ...
        if len(metals) >0:
            raise ValueError('cannot process molecule including metals')
        coords = m2.get_coordinates_in_angstrom()
        ...
        if linker_topic ==3:
            ...
            frag4_Xs = [k+frag_nums for k in frag3_Xs]
            cn_bond = []
            for x_i in center_Xs:
                x_center = coords[x_i]
        ...
        for m in range(len(labels)-3*3,len(labels)):
            if labels[m]=='C':
                ...
                for n in left_xs:
                    x_frag = coords[n]
                    if np.linalg.norm(x_frag-x_ooc) <3.5:
                        cn_bond.append((n,m))
...
def write_mapping_file(path,mapping,map_name = 'linker_ff_mapping'):
